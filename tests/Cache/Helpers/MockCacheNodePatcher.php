...
class CacheNodePatcher
{
    ...
    {
        // First, make sure Redis class is available
        self::ensureRedisMockAvailable();
    ...
    /**
     * Make sure Redis mock is available for testing
     */
    private static function ensureRedisMockAvailable(): void
    {
        ...
        // If Redis class doesn't exist, load our mock version
        $mockFilePath = dirname(__DIR__, 2) . '/Mocks/Redis.php';

        if (!file_exists($mockFilePath)) {
            throw new \RuntimeException("Redis mock file not found at: {$mockFilePath}");
        }

        require_once $mockFilePath;
    }
}
